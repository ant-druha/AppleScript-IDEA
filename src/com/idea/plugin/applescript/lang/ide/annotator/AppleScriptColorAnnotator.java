...
public class AppleScriptColorAnnotator implements Annotator {
    ...
    public void annotate(final @NotNull PsiElement element, final @NotNull AnnotationHolder holder) {
        ...
        if (element instanceof PsiReference) {
            PsiElement resolveResult = ((PsiReference) element).resolve();
            if (resolveResult == null) {
//                holder.createInfoAnnotation(element,"No references found").setTextAttributes();
                createInfoAnnotation(holder, element, AppleScriptSyntaxHighlighterColors.UNRESOLVED_REFERENCE);
            }
        }
